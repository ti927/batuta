...
## FASE FUTURA — Framework de Instrumentos (Construtor + Marketplace)  ▶️ EM EXECUÇÃO (sinal 2026-08-12) — Fatia 0+1 (motor do conector) ✅ NO AR (commit `3163721`); próxima = Fatia 2 (Construtor/telas)
...
**Faseamento estrangulador:** 0 forma/spike ✅ → **1 motor do conector (sem UI, provado por teste) ✅ NO AR** → **2 construtor (telas) ◀ próxima** → 3 importar OpenAPI → 4 IA criadora monta conector + Central → 5 marketplace básico → 6+ Nível 3 (sandbox). A Fatia 1 já entrega o valor (conector multi-operação p/ o Bubble) sem marketplace. **Sequência:** convive com a prioridade nº 1 (Unificação de Estado); quase tudo é **borda** (novo tipo + UI + criadora), toca pouco o núcleo. **Fatia 1 (commit `3163721`):** `cerebro/instrumentos/conector.py` (tipo `conector`, `expandir_ferramentas`, auth+cofre, `campos_resposta`), oculto no catálogo via `oculto_no_catalogo` até o Construtor; 11 testes novos; 2 follow-ups de motor nomeados (portão por-operação; retentativa das expandidas) — detalhe no §11 do doc-fonte.
