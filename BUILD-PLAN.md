...
### Frente A — Unificação do Runtime (remodelagem simplificadora do motor)  🔨 EM EXECUÇÃO — Fatia 1a ✅ NO AR (2026-07-26)
...
> **✅ FATIA 1 (1a) NO AR (2026-07-26, deploy `7dcdfc4`, migração `snd00sombra01`).** O rastro-sombra da conversa está em produção: cada turno conversacional vira um `PassoExecucao` de uma `Execucao` `modo='conversa'`, com entrada/saída/instrumentos/`erros_instrumentos`/uso. **100% aditivo** (`execucoes.automacao_id`→nulável, +`modo`, +`conversa_id`), sem tocar fila/sweeper/portão/laço — a sombra vive no estado próprio `'conversa'`, que os recuperadores ignoram; gravação em sessão própria à prova de falha; portão de fora. **Não precisou do descongelamento** (só a Fatia 4 precisa). 732 testes. **Falta:** teste ao vivo + **Fatia 1b** (a UI para VER o rastro — é UI, desenho proposto antes). Detalhe em `docs/REMODELAGEM-MOTOR.md §5`.
