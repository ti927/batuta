...
**Estado na Meta (App Review):** app **Publicado/Live** (App ID 1521228536398165); caso de uso "Gerenciar mensagens e conteúdo no Instagram" via **API com login do Instagram** (token colado, bate com o código); **verificação da empresa ENVIADA** sob JMF ("Em análise", ~2 dias úteis); **4 permissões submetidas** (`instagram_business_basic`, `instagram_business_content_publish`, `instagram_business_manage_comments`, `instagram_business_manage_insights`); **REMOVIDAS** `instagram_business_manage_messages` + `Human Agent` (DM/Direct — o Batuta não faz DM ainda; Meta exige vídeo demonstrando cada permissão).
