...
## FASE — Central de Conhecimento (manual do humano + base da IA)  🚧 EM ANDAMENTO (Fases 1–4 + Onda B no ar; falta só Fase 5 governança)

> **Evoluiu** da "Base de conhecimento centralizada da IA criadora": virou UMA base para DOIS leitores — a pessoa (manual em `/ajuda`, dentro do app) e a IA criadora (consulta sob demanda). Roadmap de 5 fases no plano `~/.claude/plans/a-anthropic-acabou-de-elegant-meerkat.md`. **Decisões do maestro (2026-07-17):** um documento por capítulo com seção "Para a IA"; área `/ajuda` desde já; a IA consulta via ferramenta (sem embeddings — acervo pequeno e curado).
>
> **DESCOBERTA DE DEPLOY:** o backend é construído com Root Directory = `cerebro/` → `docs/` da raiz NÃO chega ao container. Por isso o conteúdo vive em **`cerebro/central/`** (vai no deploy); o front lê via API. Fonte única: `cerebro/conhecimento.py` (parser de frontmatter próprio, sem PyYAML; `indice`/`obter`/`buscar`).
>
> **ENTREGUE (Fases 1–4):** (1) `cerebro/central/` com o `_GABARITO.md`, o `INDICE.md` (~40 capítulos em 8 áreas) e **14 capítulos da Onda A** escritos (fundamentos, hierarquia, agente, cinto, automação, gatilhos, portão, chaves-de-ia, credenciais, canal-telegram, gerar-imagem, enviar-telegram, agendar-automacao, publicar-instagram). (3) Área humana `/ajuda`: rota `cerebro/rotas/ajuda.py` (`GET /ajuda/indice`, `/ajuda/{slug}`) + `interface/app/ajuda/` (react-markdown + remark-gfm, busca por área) + item na sidebar. (4) Ferramenta **`consultar_conhecimento`** na IA criadora (`criacao/ferramentas.py`) + instrução e índice de títulos no `criacao/prompt.py`. **650 testes** (+6); tsc/eslint/build verdes. As 2 entradas-semente (Telegram; `publicar_instagram`) viraram capítulos.
>
> **ONDA B ENTREGUE (2026-07-18, commit `d71a245`):** +34 capítulos `.md` (sem código) — demais instrumentos (mídia: montar-imagem, gerar-video, gerar-video-fal, descrever-imagem, gerar-pdf; Instagram: ler-post/ler-comentarios/responder-comentario/insights; sites: wordpress, ler-site, ler-site-firecrawl; web: busca-web, busca-exa; dados: chamar-rest, banco-sql, mcp, webhook-saida), times & agentes (time, lider, memoria-do-agente, criar-com-a-ia, editar-agente), automações (cadeia-e-grafo, execucoes-e-inspecao), segredos-de-instrumento, conversas, operação (uso-e-custos, falhas-e-retentativa, parede-de-ativacao, auditoria-e-lgpd) e administração (papeis-e-permissoes, membros-e-convites, duplicar-time). Escritos da FONTE real (docstrings dos instrumentos, `auth.py`/`rotas/membros.py`, `duplicacao_time.py`). **Total: 48 capítulos navegáveis** (falta só `mensageria/canal-whatsapp`, recurso não construído). Verificado: leitor recarrega os 48, frontmatter completo, **0 links `[[..]]` quebrados**, `test_conhecimento` 6 verdes, busca da IA acha os capítulos novos. **FALTA:** Fase 5 (governança — regra de manter o capítulo em dia quando o código muda).
...
## FASE — Conversa da IA criadora em segundo plano + erros honestos em todo o app  ✅ (2026-07-19, deploys `215b613`+`e181be7`, migração aditiva `trn00turno001`, núcleo intocado)

Gatilho: conversando com a IA criadora, a conversa **caiu no meio** e a tela mostrou só "Falha ao enviar" (genérico), com a fala do usuário sumindo — e o deploy era de >24h antes (não foi reinício). Causa-raiz: o turno rodava DENTRO de um `POST /conversas-criacao/{id}/mensagens` **síncrono e bloqueante** que podia levar minutos (LLM + laço de ferramentas); qualquer timeout de proxy/oscilação de rede cortava a conexão → o `fetch` lança erro de REDE → o `catch` mostrava o texto genérico; a mensagem era só otimista, nunca persistida. **Fix (reusa o padrão da tela de execução):** o turno **enfileira e roda em segundo plano** — tabela `turnos_criacao` (migração `trn00turno001`, índice único parcial = 1 turno não-terminal por conversa) + `cerebro/fila_turnos.py` (pool de trabalhadores espelhando `fila.py`; recupera **órfãos no boot** e **presos no sweeper**; falha sempre visível e humana). `POST /mensagens` devolve 202+`turno_id` na hora; `GET …/turnos/{id}` acompanha; a leitura da conversa expõe `turno_em_andamento` (retoma após reload). Feedback ao vivo: cada ferramenta publica "o que a IA faz agora" (`atividade.registrar` no wrapper `_serial`) + `<Cronometro>`. Front (`usar-conversa.ts`/`chat.tsx`): enfileira e acompanha ~1,5s; queda de rede **reconecta** (não derruba; o turno segue no servidor); mensagem falha marca "não enviada" + **Reenviar**. **Fase B — varredura de UX:** lei nova **`CLAUDE.md §12-A`** (nada preso em request longo, nada de erro genérico, nada sem sinal de vida); `api.ts` ganhou `ErroDeRede`/`mensagemDeErro`/`mensagemPorStatus` e **23 telas** trocaram o erro genérico por mensagem honesta. Núcleo de orquestração intocado. 683 testes. Ver memória [[feedback-sem-erro-generico-operacao-longa-em-fundo]].

## FASE — Estado por automação no construtor + aba "Agendadas" nas Execuções  ✅ (2026-07-20, deploys `a303220`+`b25863d`, migração aditiva `agm00motivo001`, só borda/frontend, núcleo intocado)

No QA da conversa nova, o maestro achou bugs de agendamento (todos na borda). (1) **`a303220`** — RAIZ de um agendamento que nunca rodava: para gatilho **manual** o construtor **escondia** o controle de estado e forçava `ativa=false` em todo Salvar, mas o agendador só dispara automação **ativa** → agendar uma manual era sempre cancelado (o backend já aceitava manual+ativa; a trava era só no front). Fix: pílula do estado da própria automação + interruptor "Ativa" para qualquer gatilho; `tratar` traduz a parede (422 `{problemas}`) em texto humano. (2) **`b25863d`** — aba **"Agendadas"** nas Execuções (a aba já é "o que rodou/roda"; agendada = início do ciclo): `GET /times/{id}/agendamentos` (pendentes + cancelados 7 dias, com nome do alvo + **motivo**) + coluna `agendamentos.motivo` (migração `agm00motivo001`); o sweeper grava o motivo ao auto-cancelar (alvo desativado/removido) e o cancelamento manual grava "Cancelado por você." — §12-A, nada em silêncio. Front: filtro "Agendadas (N)", horário em Brasília, Cancelar + seção "Não dispararam". 684 testes.

## FASE — Observabilidade: banco de logs pesquisável + identidade de servidor  ✅ (2026-07-20, deploy `4ce2164`, migração aditiva `log00evento001`, só borda, núcleo intocado)

Gatilho: uma automação de produção disparou **em dobro** (07:59:57 + 08:00:00); a investigação levou horas porque **nenhum evento registrava DE ONDE veio cada disparo** — a causa era um **cérebro rodando local** (uvicorn nesta máquina desde 14/07) cujo agendador disparava a produção pelo **banco compartilhado**. Um log com "host/ambiente" teria apontado na hora. O maestro reorientou: o disparo duplo é problema de dev; a prioridade é **profissionalizar os logs** (banco pesquisável: data/hora/IP-servidor/quem/tela/comando/resultado/erro). **Construído (pacote novo `cerebro/observabilidade/`; escopo desta rodada = Fundação backend):** `log.py` (identidade de servidor host/pid/commit/`ambiente`=railway|local via presença de `RAILWAY_*`; `FormatadorJSON` no stdout; `configurar_logging` no lifespan — os 6 loggers `batuta.*` herdam; `redigir` mascara segredos), `contexto.py` (ContextVar de correlação `request_id`/quem/origem, molde `usar_chaves`/`usar_atividade` — atravessa as threads dos workers), `escritor.py` (`registrar_evento`: stdout SEMPRE + persiste `evento_log` em sessão própria **best-effort** que NUNCA derruba o request; **`ambiente=local` NÃO persiste** — dev não polui prod; política de volume: erros/escrita/disparo/execução/agendamento/mensageria/criacao persistem, GET 2xx só stdout), `middleware.py` (`MiddlewareLog`: request_id/ip[X-Forwarded-For]/rota/latência/status; devolve `X-Request-Id`; persiste só ≥400). Tabela `EventoLog` (migração `log00evento001`, 7 índices incl. GIN em `detalhe` e parcial de erros). **Instrumentação:** `disparo.criar_execucao(origem=)` grava `automacao.disparada` com **origem(manual|agendamento|webhook)+host+ambiente** (o registro que delataria o processo local); `rodar_execucao` fixa contexto e o `except` **que engolia a exceção** agora grava `execucao.falhou` **com stack**; agendador/`fila`/`fila_turnos` (órfãos/presos), `auth` (auth.negado 401/403 + fixa `usuario_id` no contexto), `mensageria` (turno.falhou/transferido), `auditoria.registrar` **espelha** em `evento_log`. **Poda** diária (info/warning 14d, error/critical 90d, em lotes). **`GET /logs`** (admin da consultoria): filtros período/nível/categoria/usuário/org/time/request_id/texto/ambiente + paginação; esconde `ambiente=local` por padrão. **Verificado em produção:** um 404 real gravou evento `http status=404 host=<container> ambiente=railway` com o mesmo `request_id` do header `X-Request-Id` (correlação ponta a ponta); bônus: os logs já pegaram varredura de bots. 693 testes (+9). Fases futuras documentadas: **telemetria do cliente** (front: hook em `requisitar()` + ErrorBoundary + `POST /logs/cliente`) e **tela de busca `/logs`** (UX proposta antes de construir). Ver [[feedback-cerebro-local-dispara-producao]].

## FASE — Migração do banco Supabase Brasil → EUA (co-locar com o Railway)  ✅ CONCLUÍDA E VALIDADA EM PRODUÇÃO (2026-07-20, sem mudança de código, núcleo intocado)

**Motivação:** o cérebro e a interface rodam no **Railway US East (Virginia)**, mas o Supabase estava em **`sa-east-1` (São Paulo)**. Cada requisição faz MUITAS consultas sequenciais (ORM) → a ida-e-volta cross-continente (~120–150 ms cada) se multiplicava e deixava o app perceptivelmente lento. O Supabase não troca a região de um projeto → a solução foi **criar um projeto novo em East US (North Virginia)** e **migrar tudo sem perder nada**. Um projeto Supabase guarda **três** coisas, e as três migraram: **Postgres (dados)**, **Auth (logins)** e **Storage (arquivos)**.

**Como foi feito (scripts operacionais `cerebro/_mig_*.py`, que leem as connection strings de `.env.migracao` gitignored — sem segredo no repo):**
- **Schema:** `alembic upgrade head` apontando pro banco novo (chega ao mesmo head `log00evento001`).
- **Dados:** cópia table-a-table via **COPY binário** (psycopg3), na ordem de FK (`Base.metadata.sorted_tables`), TRUNCATE antes (idempotente) — preserva tipos exatos (jsonb/uuid/timestamptz). **24 tabelas** conferidas linha a linha.
- **Auth preservado:** copiados `auth.users` + `auth.identities` byte-exato → **mesmos UUIDs e `encrypted_password`** → o vínculo `usuarios.auth_id` sobrevive e **a senha continua valendo** (só re-loga, pois o projeto novo tem JWKS novo). 4/4 usuários.
- **Storage:** os 84 arquivos do bucket `arquivos` copiados via Storage HTTP API (mesmos caminhos); as **URLs absolutas** `{SUPABASE_URL}/storage/v1/object/public/...` persistidas no banco (`execucoes`, `passos_execucao`, `mensagens_conversa`, etc.) **reescritas** (host antigo→novo) — 191 linhas, 0 remanescentes. Conferido: nenhuma perda (as 21 referências ausentes no novo já eram órfãs pré-existentes no antigo). Posts já publicados no Instagram não quebram (a Meta hospeda a cópia dela).

**Virada (janela curta, ninguém usando):** cérebro **parado** no Railway (congela crons/escritas) → reexecutados os scripts (cópia fresca) → trocadas as variáveis nos **2 serviços** e redeploy: **cérebro** (`DATABASE_URL` [Session pooler], `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`), **interface** (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` como **build args** → REBUILD). `COFRE_CHAVE_MESTRA` **intocada** (decifra as credenciais migradas). **Validado ao vivo:** `/saude` no ar, **login com a mesma senha**, dados todos lá, automação manual concluiu, Storage serve; **latência caiu de forma expressiva** ("muito mais rápido").
...
## PROGRAMA — Unificação de Estado (motor + IA criadora)  🔨 PRIORIDADE Nº 1 — Marco 0 ✅ + DECISÃO ✅ (dois trilhos, Frente B primeiro). **Frente B 100% COMPLETA (A–E) ✅ NO AR** (2026-07-27). **Frente A: Fatias 1–4.2 + a CURA (Fatia 4.3 — P0→P3) ✅ COMPLETA na parte de valor, NO AR e provada ao vivo** (2026-08-09) — memória entre turnos + janela/resumo + **portão nativo** (trava da conversa governada pela parede de aprovação; portão de esteira com memória); o "renasce" morreu. Corte de custo do Bubble (filtro de campos REST, −84%) e fix de arquivar imagem também no ar. **Resta:** limpar o "Confirma?" manual dos agentes (o maestro faz) + P4 (limpeza opcional). Cada fatia com aprovação, uma por vez
...
### Frente A — Unificação do Runtime (remodelagem simplificadora do motor)  ✅ A CURA ENTREGUE — Fatias 1–4.2 ✅ + a CURA (4.3: P0→P3) ✅ COMPLETA na parte de valor, NO AR e provada ao vivo (2026-08-09); resta limpar o "Confirma?" manual dos agentes (o maestro faz) + P4 (opcional)
...
> **✅ FATIA 1 COMPLETA (1a+1b) NO AR (2026-07-26).** O rastro-sombra da conversa EXISTE e é VISÍVEL. **1a** (deploy `7dcdfc4`, migração `snd00sombra01`): cada turno conversacional vira um `PassoExecucao` de uma `Execucao` `modo='conversa'` (entrada/saída/instrumentos/`erros_instrumentos`/uso); **100% aditivo** (`automacao_id`→nulável, +`modo`, +`conversa_id`), sem tocar fila/sweeper/portão/laço (estado próprio `'conversa'` que os recuperadores ignoram; escritor em sessão própria à prova de falha); **não precisou do descongelamento** (só a Fatia 4). **1b** (deploy `4505a8d`, sem migração): rastro VISÍVEL reusando a tela de Execuções — filtro **"Conversas"** + a MESMA tela de detalhe; `execucao_acessivel` escopa a sombra pela conversa→agente; novo `GET /times/{id}/conversas-rastro`. 736 testes + tsc/eslint/build. **Falta:** só teste ao vivo. Detalhe em `docs/REMODELAGEM-MOTOR.md §5`.

> **📊 MEDIÇÃO + CACHE do runtime (2026-07-26, deploy `bf36cd8`, sem migração).** O rastro da Fatia 1 permitiu MEDIR a 1ª conversa real: o peso é a **multiplicação do laço ReAct** (o `create_react_agent` reenvia ferramentas+prompt a cada passo de tool-calling — turno de 9 chamadas = 84,7k tok, ~17× o conteúdo), **não** o histórico. Ganho rápido no ar: `cache_control` no prompt de sistema do `executar_agente` (só Anthropic, guard por provedor) + captura dos campos de cache p/ o custo correto. **Medido ao vivo:** conversa nova = 48% da entrada servida do cache → **US$ 0,63 vs US$ 1,00** sem cache (**−37%**; a contagem de tokens não cai, o custo sim). Beneficia os dois motores. A **persistência entre turnos** (cura estrutural, que ELIMINA os reenvios) segue pendente — Fatias 4–5.

> **▶️ FATIAS 2 → 4.2 NO AR (2026-08-04) — detalhe em `docs/REMODELAGEM-MOTOR.md §5`.** **Fatia 2** (`4d2ab1e`): a medição do teto da conversa migra para a TIMELINE (`servico.medir_conversa`); passo passa a guardar o uso CHEIO do turno; contadores viram cache. Backend puro, sem migração, zero mudança (equivalência provada). **Fatia 3 parte segura** (`7efb6e7`): declutter da config — chaves mortas removidas (`max_passos`/`modelo_roteador`/`acao_ao_estourar`), presets 4→2 honestos, constante espelhada unificada; front data-driven (tela se atualiza sozinha). **Fatia 4 — descongelamento nº 2 AUTORIZADO** (`MIGRACAO §6.1`, 2026-08-04) + REDE DE SEGURANÇA (tag `pre-fatia-4` + dump de PROD). Sub-fatiada: **4.1** (`62e18d5`, migração aditiva `tip00passo001` — `PassoExecucao.tipo`; portão carimbado `espera_humano`; nada lê ainda → zero mudança); **4.2** (`12fb70f`, sem migração — o portão pelo CANAL passa a deixar passo `espera_humano` na timeline do fluxo, como a tela; convergência do rastro rumo a UMA timeline). **A CURA (4.3) — memória entre turnos, que mata o "renasce" — é a próxima:** decisão de arquitetura à parte (checkpointer nativo do LangGraph 1.2.2 [aceita `checkpointer`+`interrupt`, mas `PostgresSaver` não instalado] × solução caseira), com a doc oficial lida (§9) e o estudo de tokens ANTES de qualquer código.

> **✅ FATIA 4.3 = A CURA — P0→P2b NO AR e PROVADAS AO VIVO (2026-08-08).** Detalhe em [`docs/FATIA-4.3-PLANO-PRODUCAO.md`](docs/FATIA-4.3-PLANO-PRODUCAO.md) + [`docs/FATIA-4.3-DECISAO-MEMORIA.md`](docs/FATIA-4.3-DECISAO-MEMORIA.md). **Decisão de forma:** NATIVO do LangGraph (Opção A — checkpointer `PostgresSaver` + `interrupt`), escolhida pelo maestro contra a recomendação técnica (B/caseiro), com a **ampliação nº 3 do descongelamento** (`MIGRACAO §6.1`) e protótipo isolado provando memória + proteção do irreversível (+ o caveat do re-run). Plano estrangulador **P0→P4** (parada segura após P2). **✅ P0** (`0d33044`, bump de deps: `langgraph-checkpoint-postgres`+`langchain`) → **✅ P1** (`a82b5fd`, `create_react_agent`→`create_agent`, deprecado; cache Anthropic sobrevive) → **✅ P2a** (`907e00a`, memória durável entre turnos: novo `orquestracao/memoria_conversa.py` com `PostgresSaver`+`thread_id`, à prova de falha→modo legado; medição pelo DELTA; enquadramento vai p/ o prompt de sistema; semeadura do histórico no 1º turno) → **✅ P2b** (`3d1041a`+`e1b6328`, janela/resumo via `SummarizationMiddleware` nativo com resumidor Haiku só no chat, durável no checkpoint; delta por IDENTIDADE de mensagem, robusto à compactação; conserto do resumo `trim_tokens_to_summarize=None`). **PROVA ao vivo (Reembolsos): o "renasce" MORREU** — Buscar_Projetos 3×→1×, a memória persiste entre turnos. **Custo real:** a P2b revelou que o piso de custo NÃO é o histórico e sim o **resultado GIGANTE que o Bubble devolve por turno** → **✅ filtro de campos no instrumento REST** (`ba06a94`, `campos_resposta` — enxuga cada registro; drawer surge sozinho pelo schema) aplicado ao Buscar_Projetos = **−84%** (~7,1k→~1,1k tokens, provado ao vivo; turnos caíram de 28–50k p/ 15–32k). **Bugfix junto:** **✅ arquivar imagem entre turnos** (`9ddbb68`, re-baixa a foto pelo `file_id` salvo — o agente guarda o comprovante sem o cliente reenviar). Suíte **781** (à época da P2b). **✅ P3 = PORTÃO NATIVO COMPLETO e PROVADO AO VIVO (2026-08-09), suíte 799 verde:** P3a (HITL seletivo em `executar_agente`, `interrupt_on` derivado de `acao_irreversivel` — MESMA fonte da parede de ativação; commit `7d4f23f`) · P3b (TRAVA nativa na CONVERSA via `HumanInTheLoopMiddleware`; `cc05322`) · P3d (governança DEFINITIVA = **parede de aprovação** da org, não env-var por time; a IA criadora não pede confirmação em dobro; voz do agente na pausa; `b99b02e`) · P3c-B (portão de ESTEIRA com memória — TELA `ef8aa9e` + CANAL `1525ced`, Opção B conservadora). **Retrocompatível:** sem checkpointer = byte-idêntico ao legado. **Prova ao vivo:** conversa `ea7d7e26` (a trava criou 1 reembolso só após "sim") + automação `21bcb42e` (esteira pausou→aprovou→publicou no WordPress). O caveat do re-run fica CONTIDO (interrupt em `after_model`, fronteira limpa). Detalhe em [`docs/FATIA-4.3-PLANO-PRODUCAO.md`](docs/FATIA-4.3-PLANO-PRODUCAO.md). **Resta:** limpar o "Confirma?" manual dos markdowns dos agentes existentes (o maestro faz) + P4 (limpeza opcional da retomada partida). **IA criadora e Central de Conhecimento sincronizadas com todo o P3** (`b56392f`): a criadora já ensina o `campos_resposta` (filtro de custo) e a trava nativa da conversa ("não peça confirmação em dobro"); a Central ganhou a trava nativa da CONVERSA em `mensageria/conversas.md` + `operacao/parede-de-ativacao.md`. **Fila:** investigar `cpo.QuaisTrechos` (fluxo de viagem); remover o log TEMP `rest.diag`; apagar o Supabase antigo de SP (os `_mig_*.py` já apagados).

---

### Frente B — Economia de tokens da IA criadora (resumo rolante `projeto.md` + iceberg + cache)  ✅ 100% COMPLETA (A–E) NO AR (2026-07-27) — A resumo/janela, B painel "Sobre este time", C busca-histórico, D cache Anthropic, E foto enxuta
...
**Plano (5 partes, borda de criação — núcleo intocado):** **A ✅ NO AR (2026-07-26, commit `3621138`)** resumo rolante + janela dos últimos N turnos (2 colunas aditivas `ConversaCriacao.resumo`/`resumo_ate` via migração `res00rolante01`; `criacao/resumo.py` dobra os turnos antigos via Haiku em `fila_turnos`, best-effort, DEPOIS da resposta; `loop.py` envia `mensagens[resumo_ate:]` mantendo `lc` p/ preservar o padrão de tool-use + injeta o resumo no prompt; prova real −62% do histórico numa conversa de 32 turnos). **B** painel "Sobre este time" (GET/PUT do resumo). **C ✅ NO AR (2026-07-26, commit `1a8712f`)** ferramenta `buscar_no_historico(consulta)` (`criacao/ferramentas.py`) — busca nos turnos já dobrados (`mensagens[:resumo_ate]`), com teto de resultados; fecha a ressalva do resumo com perda. Backend puro, sem migração. **D ✅ NO AR (2026-07-26, commit `b28d091`)** cache de prompt Anthropic no prefixo estável — `criacao/prompt.py::montar_system_criadora`/`prompt_criadora` (cache só na Anthropic; OpenAI/Google usam texto puro) + medição cache-aware e correção do preço do Opus ($15/$75→$5/$25) em `precos.py`. Prova real: ~88% de economia por turno; 719 testes verdes. Não resolve o "abrir time frio" — isso é a Parte A. **E** fotografia enxuta (só estrutura) + `ver_agente`/`ver_automacao` sob demanda (ganho em times grandes). Migração aditiva, retrocompatível (`resumo_ate=0` = comportamento de hoje).
...
## FASE FUTURA — Framework de Instrumentos (Construtor + Marketplace)  ▶️ EM EXECUÇÃO (sinal 2026-08-12) — Fatias 0–2 ✅ NO AR: motor do conector (`3163721`) + Construtor de Instrumento (`c7db8ab`); próxima = Fatia 2.5 (biblioteca org-wide) ou Fatia 3 (importar OpenAPI)
...
**Faseamento estrangulador:** 0 forma/spike ✅ → **1 motor do conector ✅ NO AR** → **2 construtor (telas) ✅ NO AR** → **2.5 biblioteca org-wide ◀ próxima** → 3 importar OpenAPI → 4 IA criadora monta conector + Central → 5 marketplace básico → 6+ Nível 3 (sandbox). **Sequência:** convive com a prioridade nº 1 (Unificação de Estado); quase tudo é **borda**. **Fatia 1 (`3163721`):** `cerebro/instrumentos/conector.py` (tipo `conector`, `expandir_ferramentas`, auth+cofre, `campos_resposta`), oculto no catálogo até o Construtor. **Fatia 2 (`c7db8ab`, 816 testes):** `construtor-instrumento.tsx` (overlay 5 seções, botão "🌟 Criar instrumento") + backend `POST /instrumentos/{id}/testar-operacao` ("testar e detectar"). Deferido honesto: auth Basic/OAuth2, org-scope (→2.5), marketplace. 2 follow-ups de motor: portão por-operação; retentativa das expandidas. Detalhe no §11 do doc-fonte. **Aguardar teste ao vivo do maestro.**

**Decisões travadas (2026-08-12) — mockup Fase 1 VALIDADO (artefato `435b0bdf`):** (1) nome continua **"Instrumento"** + botão **"🌟 Criar instrumento"** na aba de Instrumentos do time; (2) **tipo novo** `conector` (não turbinar o REST); (3) **escopo de organização** — o instrumento criado é gravado p/ a org (todos os times) + vinculado ao time atual = **biblioteca de instrumentos da org**; (4) **substituir os instrumentos existentes** por este modelo, com a **fronteira honesta dos 3 grupos** — **(a)** nativos p/ sempre (tripas do motor: agendador/arquivar-imagem/mcp/descrever-imagem) · **(b)** migram (a maioria "só chamam API": busca/WordPress/Instagram/SearchConsole/Telegram/webhook/REST) · **(c)** esperam o **Nível 3** (precisam de código: PDF/SQL/geração de mídia); migração gradual, provada equivalente, sem quebrar instâncias vivas; (5) **credenciais moram no instrumento** — a caixa-forte de credenciais nomeadas fica redundante p/ instrumentos (o instrumento é objeto da org → credencial já é única), a **criptografia/cofre FICA**, e o pool de chaves de IA da plataforma é outra coisa e segue; (+) auth **"token auto-renovável"** (§5.3.1: o sistema renova no fundo, como o Instagram já faz). **Abertas (o "quando"):** OAuth2 login · marketplace · Nível 3/sandbox. Detalhe técnico e §§ no documento-fonte.

---

## FASE — Batuta-MCP profissional (o claude.ai opera o Batuta inteiro pela assinatura do consultor)  ✅ COMPLETA (2026-08-21) — Fatias 0–3 provadas ao vivo; Fatia 4 (acabamento) aplicada. 44 ferramentas; doc `docs/MCP-BATUTA.md`
...
- **Fatia 0 — Fundação de login real + escopo. ✅ NO AR e PROVADA AO VIVO (2026-08-21).** O maestro conectou pelo claude.ai, viu a telinha de login do Batuta, autenticou e as ferramentas escoparam certo. **2 aprendizados de deploy do 2º serviço:** (a) ele precisa de `SUPABASE_URL` (a cadeia de auth reusada exige no boot; a prova não precisava → `KeyError` no 1º deploy); (b) o conector no claude.ai tem que apontar para a URL **com `/mcp`** no fim (a raiz dá 404 "nenhum servidor MCP"). Detalhe original: `mcp_login.py` (Supabase `grant_type=password`, telinha de login com a marca, provedor OAuth 2.1 real), `mcp_escopo.py` (identidade→`Usuario`→acesso via `_comum`), `mcp_servidor.py` (FastMCP profissional + 3 tools de leitura escopadas p/ provar: `listar_organizacoes`/`listar_times`/`descrever_time`), `testes/test_mcp_login.py` (12 testes verdes offline). **Decisão de storage (revisada do plano):** em vez de tabela de tokens, os tokens são **SEM estado** (assinados HMAC via `MCP_TOKEN_SECRET` — sobrevivem a restart/réplica; revogação ao vivo por re-checagem de `Membro`, como o `auth.py`); só os **clientes OAuth** ficam numa tabela portável `mcp_cliente` criada no boot (`create_all`, fora do alembic — mesma ideia das tabelas de checkpoint do LangGraph). Env vars novas do 2º serviço: `SUPABASE_ANON_KEY`, `MCP_TOKEN_SECRET`, `MCP_PUBLIC_URL`; start vira `uv run python mcp_servidor.py`; sai `MCP_PROVA_TIME_ID`. **Nada fica aberto ao mundo depois desta fatia** (fim do auto-aprovado).
- **Fatia 1 — Leitura completa + diagnóstico. ✅ CÓDIGO PRONTO E TESTADO (sobe no próximo deploy do 2º serviço).** `mcp_ferramentas.py` (lógica de leitura; decorator `_ferramenta` = sessão+identidade+tradução de acesso numa fonte só) + 12 tools novas em `mcp_servidor.py` (total 15): `listar_agentes`/`ver_agente`/`ver_memoria_agente`, `listar_automacoes`/`ver_automacao`, `listar_execucoes`/`diagnosticar_execucao` (reusa `diagnostico_execucao.diagnosticar` — avisos+ação sugerida), `listar_conversas`/`ler_conversa`, `ver_uso` (custo por categoria via `precos.resumir_uso`), `listar_tipos_instrumento` (catálogo) e `consultar_conhecimento` (Central). Tudo escopado pelos guardas `_comum`, sem vazar segredo. 13 testes offline verdes.
- **Fatia 2 — Criação núcleo. ✅ CÓDIGO PRONTO E TESTADO (sobe no próximo deploy dos 2 serviços).** `mcp_ferramentas_escrita.py` (17 tools; decorator `_ferramenta_escrita` = sessão+identidade+commit/rollback+tradução de `ConflitoDominio`/`FalhaInstrumento`, uma fonte só) reusando `criacao/servicos.py`: `criar_time`(admin)/`editar_time`, `criar_agente`/`editar_agente`/`remover_agente`, `configurar_instrumento`/`editar_instrumento`, `montar_conector`/`testar_operacao_conector`, `encaixar`/`desencaixar`, `criar_automacao`/`renomear`/`montar_cadeia`/`definir_gatilho`/`ativar_time`/`desativar_time`. Escopado por papel (a maioria `operador`; criar time `admin`). `_validar_gatilho` e `catalogo_de_instrumentos` REUSADOS direto (são funções de módulo, não presas ao contexto da conversa) — não precisou portar. **Cura do agendador entre processos (aditiva, só na borda do cérebro):** `agendador.reconciliar()` + job de 60s — como o agendador é APScheduler EM MEMÓRIA no cérebro e o MCP roda em OUTRO processo, uma automação de gatilho `agendamento` criada/ativada pelo MCP entra no relógio em até ~1 min (o boot já sincronizava; agora reconcilia periodicamente e remove jobs órfãos). Total 32 tools; 15 testes offline + testes de fuso do agendador verdes. **A mudança do agendador exige redeploy do CÉREBRO** (as tools MCP exigem redeploy do 2º serviço; ambos auto-deploy do push).
- **Fatia 3 — As áreas "tudo".** Feita POR PARTES.
  - **Fatia 3a — Credenciais + chaves de IA. ✅ CÓDIGO PRONTO E TESTADO (sobe no próximo deploy do 2º serviço; NÃO toca o cérebro).** **Decisão de produto/segurança do maestro:** o Claude NUNCA recebe segredo — ele cria o ESQUELETO (nome+tipo) e lê (mascarado); o SEGREDO o consultor cola no cofre do Batuta pela tela ("a IA nunca pluga o token"). 5 tools novas (total 37): leitura `listar_tipos_credencial`/`listar_credenciais` (mascarado + `usado_por` + `preenchida`)/`ver_chaves_de_ia` (só quais provedores têm chave, por EXISTÊNCIA — não decifra, então o MCP NÃO precisa da chave-mestra do cofre; least-privilege); escrita `criar_credencial` (esqueleto, `montar_resumo(tipo,{})`, sem segredo, operador)/`remover_credencial` (operador, bloqueia se em uso). **Decisão de risco:** para não mexer nas rotas sensíveis de credenciais/chaves (produção usa; sem test-DB local), o MCP REUSA os primitivos já compartilhados (`tipos_credencial`, `credenciais_cofre`, models) sem refatorar as rotas — o núcleo de segredo (cofre) já é fonte única e o MCP nunca o toca. 16 testes offline verdes.
  - **Fatia 3b — config-memória, referência a credencial, exclusão, duplicação, organização. ✅ CÓDIGO PRONTO E TESTADO (sobe no próximo deploy do 2º serviço; NÃO toca o cérebro).** 7 tools novas (total 44): `configurar_memoria_agente` (liga/desliga a memória do agente, direto no ORM como a rota faz), `apontar_credencial` (instrumento→credencial, reusa `rotas.instrumentos._validar_credencial`), `duplicar_time` (reusa `duplicacao_time.duplicar_time`, admin), `excluir_time`/`excluir_automacao`/`excluir_instrumento` (admin, irreversível, cascade; o reconcílio da Fatia 2 tira do relógio a automação apagada), `criar_organizacao` (replica org+Membro admin). Sem tocar o cérebro: reusa funções já compartilhadas e cascade dos deletes. **DEFERIDO (honesto):** gestão de MEMBROS (convites mandam e-mail = outward + precisa de `SERVICE_ROLE_KEY`), duplicação de AUTOMAÇÃO (lógica de clone é inline na rota, ~50 linhas — replicar arrisca divergência), e `icone` de instrumento (cosmético). 17 testes offline verdes.
- **Fatia 4 — Polimento profissional + ops + Central. ✅ CÓDIGO PRONTO.** **Erro honesto** (§12-A): catch-all nos 2 decorators — nenhum stack trace chega ao Claude; o erro real vai para o log do servidor (`batuta.mcp`). `instructions` do servidor reescrito para o toolset completo + regras de segurança (papéis, "a IA nunca pluga segredo", parede, exclusão irreversível). Auditoria `origem="mcp"` nas escritas. **Central de Conhecimento:** capítulo novo `cerebro/central/operacao/operar-pelo-claude-mcp.md` (descoberto pelo auto-scan; consultável pela criadora E pelo MCP). Doc `docs/MCP-BATUTA.md` (arquitetura/tools/segurança/deploy). **Prova aposentada:** `mcp_prova.py`/`mcp_auth.py` REMOVIDOS (nada os importava; o serviço roda `mcp_servidor.py` desde a Fatia 0); banner de "superada" no `docs/MCP-PROVA.md`. Este commit toca o cérebro só pelo capítulo da Central (conteúdo) → os 2 serviços redeployam. 17 testes offline verdes.
...
**Verificação:** 955 testes verdes (13 novos) + tsc/eslint/build da interface + **prova de fogo real**: banco local derrubado com o vigia ativo → elos caem com erro traduzido em ≤30 s, auto-cura tenta e relata que não curou, banco volta → auto-cura reconecta e os elos voltam ao verde, com a sequência completa de eventos. Em produção, `/saude` do commit `6227b12` respondeu com os campos novos zerados (`elos_caidos: []`, `elos_degradados: []`) — o vigia sondando de verdade.

**✅ TESTE AO VIVO DO MAESTRO (2026-08-27):** a página `/status` foi **aprovada** e o **time de Reembolsos voltou a funcionar** pelo Telegram. Fecha o ciclo: o que parecia "código quebrado" era a rede, e o que ficou no lugar é a blindagem + a visibilidade.

**A lição virou lei (`CLAUDE.md §12-A`, bullet novo):** *nenhuma ligação sem limite de espera, e nenhuma queda sem endereço* — toda saída para fora do processo nasce com timeout (inclusive contra a **conexão congelada**, que nenhum `connect_timeout` pega), com sonda e com nome na página de status. Também corrigidos os números velhos na Central (o vigia agora tem duas medidas: ~8 min no atendimento, ~30 no portão).
