...
8. **`docs/CEREBRO-PLANO.md`** — o **Cérebro da organização**: Quadros (no ar desde 2026-09-24), depois fila/gatilhos, Arquivos e Biblioteca. Leia antes de mexer em quadros, no instrumento `quadro`, nos links de leitura ou em qualquer coisa que agentes compartilhem entre si. **Regra de fronteira:** se a empresa já tem um sistema oficial para o dado, a fonte é ele — o quadro guarda o trabalho dos agentes.
...
>
> **E leia `docs/FALHAS-DO-MOTOR.md` antes de mexer em espera-por-humano, retomada ou mensageria.** É o catálogo dos 51 modos de falha do ciclo de vida de uma execução, com o que já está coberto e o que não está — escrito depois do incidente de 2026-09-21, em que a tela e o canal responderam a mesma aprovação ao mesmo tempo, bifurcaram o checkpoint do LangGraph e mataram uma execução de quase 4 h. A regra que saiu de lá e vale para peça nova: **estado que pertence à execução mora na execução** (a trava, o relógio, o endereço, o dono do fio) — se morar só na borda, some quando a borda não estiver lá.
...
- **Fail-safe não pode ser mudo.** Um `except` que protege o atendimento (cai para modo legado, segue sem a peça que falhou) é correto — **e insuficiente sozinho**. Em agosto de 2026 essa proteção escondeu duas falhas graves: o checkpointer da memória de conversa caiu e ficou **três dias** invisível (levando junto a trava de ação irreversível), e um turno de aprovação começou e nunca voltou, deixando a conversa presa **~1 h em silêncio**. Nos dois casos o app parecia saudável. Então todo caminho degradado precisa das **três** pernas: **(a)** evento no banco de logs (`registrar_evento`, nível `error`) — o log do servidor não conta, ninguém o abre; **(b)** um **vigia** que destrave o que ficou preso (nenhum estado "em andamento" pode ficar sem quem o varra); **(c)** **recado honesto a quem estava esperando**, com o que fazer. E o rastro precisa **carimbar o modo** (ex.: `memoria: duravel|legado`), senão não há como saber depois se a execução rodou degradada.
- **Falha devolvida como dado também é falha.** Quando uma ferramenta responde `ok: false` em vez de levantar exceção, o agente decide sozinho como seguir — e com frequência narra sucesso. Registre a falha crua no rastro: **o texto do agente nunca é prova de que a ação aconteceu.**
- **Nenhuma ligação sem limite de espera, e nenhuma queda sem endereço.** Toda saída para fora do processo (banco, HTTP, provedor de IA, canal) precisa de teto — inclusive para o modo de falha mais traiçoeiro, a **conexão congelada**, que não é recusa nem lentidão: os bytes ficam parados em trânsito, sem erro e sem fechamento, e nenhum `connect_timeout` a pega (no Postgres quem corta é `tcp_user_timeout` + keepalive + `statement_timeout`). Em 2026-08-27 a rede até o pooler congelou ~30 min: como o engine principal não tinha nenhum limite, um turno ficou **31 min pendurado**, todo o atendimento emudeceu e o `/saude` continuou verde. E descobrir **onde** quebrou não pode depender de inspeção manual do banco — por isso cada ligação virou um **elo** sondado ativamente (`saude_elos.py`), com a página **`/status`** dizendo qual caiu, evento no banco de logs a cada transição (`elo.caiu`/`elo.voltou`/`elo.reconectado`) e reconexão (automática nos elos de banco, por botão nos demais). Regra permanente: **peça nova que fala com o mundo lá fora nasce com timeout, com sonda e com nome na página de status.**
