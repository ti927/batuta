...
1. **O motor evolui por decisão dirigida — nunca por reescrita cega.** *(Princípio atualizado em 2026-07-26; substitui o "núcleo intocável" original, cujo custo estrutural está explicado na "Evolução de governança" ao final deste item.)* A orquestração de agentes, o LangGraph, a espera-por-humano, os gatilhos, a tela de inspeção, os instrumentos já construídos são o **core validado — precioso e protegido**. A regra **padrão** continua sendo **estender e adicionar**, não refatorar por impulso. Mas quando o produto **exige** mudar o motor (e a experiência mostrou que exige), a mudança é **autorizada de forma formal, aditiva, faseada e aprovada-antes**, com rede de testes máxima — jamais um rewrite às cegas. Se uma fase parece exigir mudança no núcleo, **pare, dimensione o impacto e alinhe com o maestro** um aditivo curto a esta seção antes de prosseguir (o formato do precedente de 2026-06-16).
   ...
   - **Ampliação da suspensão nº 3 — Fatia 4.3, adoção do nativo (2026-08-06, autorizada pelo maestro):** feito o estudo de arquitetura ([`docs/FATIA-4.3-DECISAO-MEMORIA.md`](docs/FATIA-4.3-DECISAO-MEMORIA.md), doc oficial do LangGraph lida + ambiente verificado), o maestro **decidiu pela Opção A — persistência NATIVA** (checkpointer `PostgresSaver` + `interrupt()`/HITL nativo), ciente das ressalvas (recomendação técnica era a Opção B/caseira). Isso **amplia** a suspensão nº 2 num ponto que ela preservava congelado: o **laço `create_react_agent`** em `orquestracao/agente.py` pode agora ser adaptado para receber `checkpointer` + `thread_id` e para o portão nativo (`interrupt()`), **sob duas condições inegociáveis**: **(a)** a **garantia HITL de nunca re-disparar ação irreversível** (`PRODUTO §14`) **continua absolutamente congelada** — o modelo nativo (que re-roda o nó ao retomar) só é adotado se o **HITL SELETIVO** (interromper apenas instrumentos irreversíveis) a proteger, o que é **provado num protótipo isolado ANTES** de tocar a produção; se o protótipo não provar a proteção, a decisão volta à mesa; **(b)** permanecem congelados os demais itens da lista (`seguir_para`, contrato de instrumentos, fila `FOR UPDATE SKIP LOCKED`, heartbeat/sweeper/recuperação de órfãos-presos §12-A). Autorizada também a **instalação das dependências** `langgraph-checkpoint-postgres` e `langchain` (`HumanInTheLoopMiddleware`). Execução na produção segue estranguladora, sub-fatiada, com a rede `pre-fatia-4` + dump de PROD já montada.

   - **Descongelamento nº 4 — a garantia HITL sai do MOTOR e vira INSTRUMENTO (2026-08-31, decisão do maestro):** as suspensões nº 2 e nº 3 mantinham *"a garantia HITL antes de ação irreversível"* na lista do que **jamais** se toca. Ela **está descongelada** — não porque a garantia deixou de importar, mas porque **mudou de lugar**, a pedido explícito do maestro: *"ACABA COM ESSA MERDA, TODA APROVAÇÃO É FEITA PELO AGENTE E PONTO FINAL. RETIRA ESSA MERDA DE PORTÃO E PAREDE."*

     **O que existia e por que caiu.** O Batuta tinha **duas** travas de aprovação coexistindo, e o maestro passou **meses sem saber que eram duas**: o **portão** (`no.gate`, um interruptor num nó do desenho) e a **parede** (`organizacoes.parede_ativacao`, uma chave da organização que recusava ativar uma automação cujo agente irreversível não tivesse portão antes — e que, desde a Fatia 4.3/P3b, ainda ligava a trava nativa da conversa). O resultado foi o oposto do pretendido: o mesmo atendimento pedia confirmação **duas vezes**, o desenho do fluxo mentia sobre o que o motor faria, e uma decisão que é do agente vivia num interruptor escondido.

     **O que ficou no lugar.** O instrumento **`pedir_aprovacao`** ("Pedir aprovação e aguardar"), no cinto do agente: ele apresenta, a execução **para** (`aguardando_humano`, exatamente a mesma máquina de pausa/retoma de sempre), e o **mesmo agente** continua quando a pessoa responde. Quem decide que aquele momento precisa de gente é o agente, porque o **markdown dele** manda — a doutrina de `CLAUDE.md §14` ("todo comportamento vem dos quatro textos") passa a valer também aqui.

     **O que foi removido:** `cerebro/portao_ativacao.py`, `organizacoes.parede_ativacao`, `instrumentos.exige_aprovacao` (coluna morta), o campo `gate` do nó e seus satélites (`no.aprovacao`, `no.instrucoes`/"portao.md"), o portão nativo da conversa (`HumanInTheLoopMiddleware`, `memoria_conversa.ha_interrupcao`) e as telas/validações correspondentes. **NÃO** foi removido `instrumentos/base.py::acao_irreversivel`: ele continua governando a **política de falha** (`PRODUTO §16`) e o selo do catálogo — deixou de ser uma trava de ativação e passou a ser um **aviso** a quem monta o time.
   ...
   - **Ampliação nº 6 — operação, tempo e composição (2026-09-02/04, autorizada pelo maestro):** com as ondas 1 e 2 no ar, o maestro mandou seguir e **inverteu a ordem do plano com aval explícito** — a **Onda 4 (operação e confiança)** primeiro, porque resolvia dor que já estava acontecendo (um artigo morreu sem poder ser retomado; cinco automações de blog quase passaram a falhar todo dia em silêncio), e a **Onda 3 (tempo e composição)** depois. Fica autorizada, de forma **aditiva**, a continuação da evolução dirigida do motor nas duas ondas. **✅ CONCLUÍDA em 2026-09-04** com a fatia 4 da Onda 3 (nó "Chamar outra automação", `1465e7b`, migração `sub00chamada01`): a frente inteira — Ondas 1, 2, Parte III, 3 e 4 — está no ar, faltando apenas o teste ao vivo do maestro. Nenhuma das mudanças tocou os itens que a suspensão preserva congelados: a garantia HITL segue no instrumento `pedir_aprovacao` (Descongelamento nº 4), e o contrato de instrumentos, o `seguir_para`, a fila `FOR UPDATE SKIP LOCKED` e o heartbeat/sweeper §12-A continuam intactos — este último, aliás, **reforçado**: o elo "Vigia das execuções" (`3d9a86a`) passou a vigiar os próprios vigias, porque as pausas novas dependem deles para voltar.

     **O que entrou (tudo aditivo, e cada peça com o seu porquê):**
     - **Onda 4, completa.** `execucoes.desenho` (a execução roda a foto do grafo tirada no disparo — editar a automação deixou de mudar o caminho no meio da corrida); `execucoes.no_inicial`/`origem_execucao_id` ("rodar de novo a partir daqui", herdando desenho, ficha e a entrada exata do passo); `execucoes.origem` + `automacoes.desligada_por_falhas_em`/`falhas_contam_desde` (o **disjuntor**: automação que falha **sozinha** 3× seguidas se desliga e avisa — disparo manual não conta, e falha causada pelo próprio Batuta também não); teto de **custo** por execução na cascata do fluxo; `execucoes.teste_de_no` ("testar este passo", com os instrumentos **reais** — não existe modo de mentira, porque fingir enganaria sobre o que o teste prova).
     - **Onda 3, três de quatro fatias.** O vigia de execuções presas passou a contar `atividade_em` como **sinal de vida** (e os instrumentos de vídeo passaram a **bater** durante a espera, em vez de encolher o próprio teto para fugir do vigia); módulo novo `orquestracao/prazo.py` com **tetos de tempo** por passo (com ajuste por nó) e por execução; e o tipo de nó **`esperar`** + `execucoes.retomar_em` + o estado `aguardando_tempo`.

     **A decisão de desenho que vale registrar:** o nó "Esperar" **não** ganhou máquina própria. Ele reusa a pausa/retomada que a aprovação já usava (`pendencias` + ficha), trocando apenas quem solta — de uma pessoa para um vigia do relógio. Foi por isso que uma capacidade grande coube numa fatia pequena.

   - **Ampliação nº 7 — nenhum teto secreto, nenhum canal surdo (2026-09-14, `e4e498b`+`d865395`):** aditiva e nascida de falha em produção (execução `3a1edfd6`), não de plano. O núcleo (`cadeia.py`) **não foi tocado**: o que mudou foi quem o chama passar `max_passos` da cascata, em vez de aceitar o fixo — o limite existia igual, só era invisível. O resto vive todo na **borda** (`mensageria/`): o teto de custo da conversa deixou de contar trabalho de instrumento, o portão passou a usar a régua da tela (`portao_max_rodadas`) em vez de turnos/custo da conversa, e atingir um limite deixou de emudecer o canal. **Reforça** — não afrouxa — os itens congelados: a garantia HITL do `pedir_aprovacao` ficou **mais** forte (um pedido novo religa o canal, e resposta a portão pendente é sempre lida, onde antes era engolida), e o §12-A ganhou o vigia do último estado que não tinha nenhum (`humano_assumiu`). Da lei do maestro saiu regra permanente, registrada em `PRODUTO.md §21`: **todo limite é configurável, visível antes de doer, e se explica quando dispara.**

     **O que a suspensão cobre:** `orquestracao/cadeia.py` (o nó `esperar`, os tetos, o `so_um_passo`), `orquestracao/disparo.py` (o funil `criar_execucao` ganhou os parâmetros herdados; retomada por pendências), `orquestracao/agente.py` (o prazo entrou no `_turno_interrompido`, o ponto único que já parava o turno), `orquestracao/grafo.py` (tipo `esperar`), `fila.py` (o terceiro sinal do vigia + o vigia das esperas), módulos novos `orquestracao/circuito.py` e `orquestracao/prazo.py`, `modelos.py` e as migrações aditivas `des00desenho01`, `rex00reexecucao01`, `cir00circuito01`, `tst00testeno01`, `esp00esperar01`.

     **O que permanece congelado:** a mesma lista de sempre — `seguir_para`, o contrato de instrumentos, a fila `FOR UPDATE SKIP LOCKED`, o laço do agente e o heartbeat/sweeper/recuperação de órfãos-presos (§12-A). O vigia de presas **ganhou um sinal a mais**, não uma regra nova: continua matando só o que ficou sem progresso além do teto.

     **Estado do plano:** falta a **fatia 4 da Onda 3** — o nó "Chamar outra automação" (sub-fluxo síncrono que devolve o resultado ao chamador). Detalhe fatia a fatia no `BUILD-PLAN.md`.
