...
> **Status:** ✅ **A CURA ENTREGUE.** Fatias **1 → 4.2 NO AR** e a **Fatia 4.3 (memória entre turnos + portão nativo, P0→P3c-B) COMPLETA na parte de valor, NO AR e PROVADA AO VIVO (2026-08-09)** — ver §5. O "renasce" morreu; o portão da conversa e o da esteira retomam do estado salvo. Cada fatia começou só com sinal explícito do maestro; a Fatia 4 exigiu suspensão dirigida do congelamento do núcleo (descongelamentos nº 2 e nº 3, `MIGRACAO §6.1`; seção 7). **Resta:** limpar o "Confirma?" manual dos agentes existentes (o maestro faz) + P4 (limpeza opcional da retomada partida).
>
...
> - **A cura ESTRUTURAL — persistência entre turnos (parar de re-buscar o mesmo dado a cada turno) — ✅ ENTREGUE na Fatia 4.3 (2026-08-09):** o checkpointer nativo (`PostgresSaver`+`thread_id`) guarda o fio inteiro; o agente retoma do estado salvo em vez de re-derivar. ELIMINA os reenvios (o cache só os barateava). Provado ao vivo: Buscar_Projetos 3×→1×. Ver §5 (4.3).
...
> - **4.3 — A CURA: memória entre turnos (o agente do portão retoma do estado salvo, não re-deriva).** Risco alto. Decisão de arquitetura à parte (checkpointer nativo do LangGraph × solução caseira) — trazida com a doc oficial lida (§9) e o estudo de tokens, ANTES de qualquer código. É aqui que o "renasce" morre — e o lugar natural de unificar a EXECUÇÃO do re-run (não só o rastro). **🟡 ESTUDO/DECISÃO PRONTO (2026-08-06): [`FATIA-4.3-DECISAO-MEMORIA.md`](FATIA-4.3-DECISAO-MEMORIA.md)** — doc oficial lida + ambiente verificado (`PostgresSaver` e `langchain`/middleware HITL AUSENTES; `interrupt()` re-roda o nó → colide com a garantia de não re-disparar instrumento irreversível, §7). Três opções (nativo × **caseiro incremental** × híbrido); recomendação técnica era caseiro. **✅ MAESTRO DECIDIU: NATIVO (A), 2026-08-06** (ampliação de descongelamento nº 3, `MIGRACAO §6.1`). Protótipo isolado rodado (produção intocada): memória entre turnos FUNCIONA; ação irreversível protegida pelo `interrupt()`-antes-do-efeito; **caveat do re-run CONFIRMADO** (o que roda antes do portão re-executa → só idempotente antes do gate); `create_react_agent` DEPRECADO → migra p/ `create_agent`+`HumanInTheLoopMiddleware`; instalar `langchain` sobe o core (regressão obrigatória). **Plano de produção:** [`FATIA-4.3-PLANO-PRODUCAO.md`](FATIA-4.3-PLANO-PRODUCAO.md) (`P0`→`P4`, estrangulador, parada segura após `P2`). **✅ P0→P3 NO AR e PROVADAS AO VIVO (2026-08-09):** P0/P1 (`create_react_agent`→`create_agent`+deps) · P2a/P2b (memória durável + janela/resumo — o "renasce" morreu) · **P3 = portão nativo** — P3a (HITL seletivo em `executar_agente`), P3b (TRAVA na CONVERSA), P3d (governança DEFINITIVA = **parede de aprovação** da org; fim da confirmação em dobro; voz do agente), P3c-B (portão de ESTEIRA com memória, tela **e** canal). Provado: conversa `ea7d7e26` (trava criou 1 reembolso só após "sim") + automação `21bcb42e` (esteira pausou→aprovou→publicou no WordPress). **Caveat do re-run CONTIDO** (interrupt em `after_model`, fronteira limpa). **Resta:** limpar o "Confirma?" manual dos markdowns (o maestro faz) + P4 (opcional).
>
