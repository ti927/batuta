...
- **Cadeia por BIFURCAÇÃO** (`orquestracao/cadeia.py`): a `cadeia` (JSONB da automação) é um grafo onde cada agente, ao terminar, **escolhe uma de várias saídas** (roteamento por LLM estruturado); loops são permitidos (com guarda de máximo de passos). Não é uma fila linear.
- **Espera-por-humano** (a peça mais delicada): um nó pode ter `pausa_humano`. A execução **pausa** (estado `aguardando_humano`, salvo no banco — sobrevive a reinício) e **retoma** quando um humano responde via `POST /execucoes/{id}/responder`. No **portão de aprovação**, a *resposta do humano* é que escolhe a saída do nó (aprovado→ramo X; reprovado→ramo Y). Hoje a espera é respondida **na tela** (o WhatsApp como canal é fase futura).
