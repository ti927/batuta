...
function TabAutomacoes({ onToast }) {
  const A = window.CT.automacao;
  const [gatilho, setGatilho] = useStateE(A.gatilho);
  const gatilhos = [
    { key: 'Manual', label: 'Manual (botão)', hint: 'Dispara apenas pelo botão de teste, na tela da automação.', icon: 'play' },
    { key: 'Agendamento', label: 'Agendamento', hint: 'Roda sozinho num horário fixo (ex.: seg–sex às 8h).', icon: 'clock' },
    { key: 'Webhook', label: 'Webhook', hint: 'Um sistema externo dispara via URL (ex.: WhatsApp, CRM).', icon: 'zap' },
  ];
  return (
    <div style={{ padding: '24px 0 60px' }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
        <div><h2 style={{ fontSize: 17, fontWeight: 500 }}>{A.nome}</h2><p style={{ fontSize: 13.5, color: '#6B6880', marginTop: 3 }}>Como a tarefa entra, por quais agentes passa e quando para pra você decidir.</p></div>
        <div style={{ flex: 1 }} />
        <button className="btn-primary" style={{ height: 38 }} onClick={() => onToast('Automação salva')}><Icon name="check" size={16} /> Salvar</button>
      </div>

      {/* Gatilho */}
      <SectionLabel icon="zap">Gatilho — o que inicia este fluxo</SectionLabel>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {gatilhos.map(g => (
          <button key={g.key} onClick={() => setGatilho(g.key)} style={{ flex: 1, minWidth: 220, textAlign: 'left', cursor: 'pointer', font: 'inherit', background: gatilho === g.key ? '#F4F1FE' : '#fff', border: '1px solid ' + (gatilho === g.key ? '#6D4AFF' : '#E8E6F0'), borderRadius: 10, padding: 14, display: 'flex', gap: 11, alignItems: 'flex-start' }}>
            <span style={{ width: 34, height: 34, borderRadius: 9, background: gatilho === g.key ? '#6D4AFF' : '#EFEAFF', display: 'grid', placeItems: 'center', flex: 'none' }}><Icon name={g.icon} size={17} color={gatilho === g.key ? '#fff' : '#6D4AFF'} /></span>
            <div><div style={{ fontSize: 14, fontWeight: 500 }}>{g.label}</div><div style={{ fontSize: 12, color: '#6B6880', marginTop: 2, lineHeight: 1.45 }}>{g.hint}</div></div>
          </button>
        ))}
      </div>

      {/* Saídas dos agentes */}
      <SectionLabel icon="layers">A cadeia — saídas de cada agente</SectionLabel>
      <p style={{ fontSize: 13, color: '#6B6880', marginTop: -4, marginBottom: 14, lineHeight: 1.5 }}>Para cada agente, defina o rótulo da saída e o destino (outro agente ou o fim). O portão de aprovação faz o fluxo parar e esperar sua decisão.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {A.saidas.map((s, i) => (
          <div key={i} style={{ background: '#fff', border: '1px solid #E8E6F0', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 10 }}>
              <RobotFace color={(window.CT.agentes.find(x => x.nome === s.agente) || {}).cor || '#B19CD9'} size={26} />
              <span style={{ fontSize: 14.5, fontWeight: 500 }}>{s.agente}</span>
              {s.inicial && <span style={{ fontSize: 11, color: '#3D2A99', background: '#EFEAFF', padding: '1px 7px', borderRadius: 999, fontWeight: 500 }}>inicial</span>}
              <div style={{ flex: 1 }} />
              <button style={{ fontSize: 12.5, color: '#6D4AFF', background: 'transparent', border: 'none', cursor: 'pointer', font: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}><Icon name="plus" size={13} /> saída</button>
            </div>
            {s.gate && (
              <div style={{ display: 'flex', gap: 9, alignItems: 'flex-start', background: '#FDF8EE', border: '1px solid #F0E2C0', borderRadius: 8, padding: '9px 11px', marginBottom: 10 }}>
                <span style={{ width: 18, height: 18, borderRadius: 4, background: '#E89638', display: 'grid', placeItems: 'center', flex: 'none', marginTop: 1 }}><Icon name="check" size={12} color="#fff" /></span>
                <span style={{ fontSize: 12.5, color: '#8a6418', lineHeight: 1.45 }}>Portão de aprovação ligado — ao terminar, pausa e espera sua decisão antes de seguir.</span>
              </div>
            )}
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input defaultValue={s.rotulo} style={{ flex: 1, border: '1px solid #E8E6F0', borderRadius: 8, padding: '9px 12px', fontSize: 13.5, fontFamily: 'inherit', color: '#1A1730', outline: 'none' }} />
              <Icon name="arrowRight" size={16} color="#A09DB8" />
              <div style={{ minWidth: 170, border: '1px solid #E8E6F0', borderRadius: 8, padding: '9px 12px', fontSize: 13.5, color: s.destino === 'fim' ? '#3DAA5C' : '#1A1730', background: '#FAFAF7', display: 'flex', alignItems: 'center', gap: 6 }}>
                {s.destino === 'fim' ? <><Icon name="checkCircle" size={14} color="#3DAA5C" /> fim</> : <><RobotFace color={(window.CT.agentes.find(x => x.nome === s.destino) || {}).cor || '#B19CD9'} size={18} /> {s.destino}</>}
              </div>
            </div>
          </div>
        ))}
