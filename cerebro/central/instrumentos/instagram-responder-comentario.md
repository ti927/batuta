...
3. Como é **ação irreversível** (age publicamente), coloque um **portão de aprovação no passo anterior**.

## Exemplos
- [ler o comentário + redigir a resposta → **portão**] → [responder].
- Um fluxo que oculta automaticamente comentários com palavras proibidas (com aprovação antes, se quiser).

## Limites e cuidados
- É `acao_irreversivel = true` → exige [[automacoes/portao-de-aprovacao]] antes.
...
(`responder`/`ocultar`/`reexibir`/`apagar`) e `mensagem` (só para responder). Monte com portão antes; o nó
